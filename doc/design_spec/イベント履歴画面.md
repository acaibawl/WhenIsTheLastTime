...
- 最新の履歴エントリの `executedAt` が `lastExecutedAt` になる
...
- 削除後に残っている最新の履歴エントリの `executedAt` が `lastExecutedAt` になる
- すべての履歴を削除した場合、`lastExecutedAt` はイベント作成日時になる

...
CREATE TABLE events (
  ...
  last_executed_at TIMESTAMP NOT NULL COMMENT '最終実行日時',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '作成日時',
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新日時',
  PRIMARY KEY (id),
  INDEX idx_events_user_id (user_id),
  INDEX idx_events_last_executed_at (last_executed_at),
...
| last_executed_at | TIMESTAMP | NO | - | 最終実行日時（履歴の最新日時） |
| created_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | レコード作成日時 |
| updated_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | レコード更新日時 |
...
| idx_events_last_executed_at | INDEX | last_executed_at | 経過時間でのソート・フィルタリング |
...
```
- 使用インデックス: `idx_events_user_id`, `idx_events_last_executed_at`

#### パターン2: カテゴリーフィルタリング
```sql
SELECT * FROM events 
WHERE user_id = ? AND category_icon IN (?, ?, ?)
ORDER BY last_executed_at DESC;
```
...
#### カバリングインデックス
頻繁に使用されるクエリに対しては、カバリングインデックスの追加を検討：

```sql
-- イベント一覧取得の最適化
CREATE INDEX idx_events_user_id_last_executed_at_name 
ON events(user_id, last_executed_at, name);
```
...
#### last_executed_at の自動更新

履歴が追加・更新・削除された際に、親イベントの `last_executed_at` を自動更新：
...
DELIMITER $$
CREATE TRIGGER trg_histories_after_update
...
-- 履歴削除時
DELIMITER $$
CREATE TRIGGER trg_histories_after_delete
AFTER DELETE ON histories
FOR EACH ROW
BEGIN
  UPDATE events 
  SET last_executed_at = (
    SELECT COALESCE(MAX(executed_at), created_at)
    FROM histories 
...
-- イベント作成
INSERT INTO events (id, user_id, name, category_icon, last_executed_at) 
VALUES (?, ?, ?, ?, ?);

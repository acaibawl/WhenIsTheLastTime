...
### 1.2 目的
新しいイベントの作成、または既存イベントの編集を行う画面。

- **作成モード**: イベント名、カテゴリーアイコン、初回実行日時、メモを入力・設定できる
- **編集モード**: イベント名、カテゴリーアイコンのみ編集可能（最終実行日時は履歴から自動算出）
...
**注意:** 編集モードでは最終実行日時は表示しません。最終実行日時は履歴から自動算出されるため、履歴画面で管理します。
...
interface CreateEventRequest {
  ...
  executedAt: string; // ISO 8601形式（初回実行日時）
  memo?: string;      // メモ（任意）
}
...
**注意:** 更新時には最終実行日時は送信しません。最終実行日時は履歴から自動算出されます。
...
export const useEventForm = (mode: 'create' | 'edit', eventId?: string) => {
  ...
  const selectedDateTime = ref(new Date());  // 作成モードのみ使用
  const memo = ref('');                       // 作成モードのみ使用
    ...
    try {
      if (mode === 'create') {
        const payload = {
          ...
          executedAt: selectedDateTime.value.toISOString(),
